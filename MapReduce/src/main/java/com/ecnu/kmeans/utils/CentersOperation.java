...
import static com.ecnu.kmeans.utils.FileOperation.*;

/**
 * 用于完成 KMeans 中心数据的有关操作
 * @author ikroal
 * @date 2019-06-14
 * @version: 1.0.0
 */
public class CentersOperation {
    ...
    public static List<List<Double>> getCenters(String centersPath, boolean isDirectory) {
        ...
        try {
            if (isDirectory) {
                List<String> paths = getPaths(centersPath);
                if (paths == null) {
            ...
            InputStream inputStream = read(centersPath);
    ...
    public static boolean compareCenters(String centerPath, String newCenterPath) {
        ...
        if (distance == 0) {
            ...
            deletePath(newCenterPath, true);
            return true;
        } else {
            //中心数据不相同则要将新的中心数据复制到初始中心数据路径处
            List<String> paths = getPaths(newCenterPath);
            try {
                if (paths == null) {
                    throw new Exception("centers directory is empty");
                }
                for (String path : paths) {
                    InputStream inputStream = read(path);
                    OutputStream outputStream = write(centerPath, true);
